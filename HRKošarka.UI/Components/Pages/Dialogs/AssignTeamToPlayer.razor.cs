using HRKošarka.UI.Components.Base;
using HRKošarka.UI.Contracts;
using HRKošarka.UI.Services.Base;
using HRKošarka.UI.Services.Base.Common;
using HRKošarka.UI.Services.Base.Common.Requests;
using Microsoft.AspNetCore.Components;
using MudBlazor;

namespace HRKošarka.UI.Components.Pages.Dialogs
{
    public partial class AssignTeamToPlayer : PermissionBaseComponent
    {
        [CascadingParameter] public IMudDialogInstance MudDialog { get; set; } = default!;
        [Parameter] public Guid PlayerId { get; set; }

        [Inject] public IPlayerService PlayerService { get; set; } = default!;
        [Inject] public ITeamService TeamService { get; set; } = default!;
        [Inject] public ISeasonService SeasonService { get; set; } = default!;

        private List<AvailableTeamDTO> _teams = new();
        private List<SeasonDTO> _seasons = new();
        private AvailableTeamDTO? _selectedTeam;
        private SeasonDTO? _selectedSeason;
        private DateTime? _joinDate;
        private int? _jerseyNumber;
        private string _searchTerm = string.Empty;
        private bool _loadingTeams;
        private bool _loadingSeasons;
        private bool _saving;

        private DateTime? MinJoinDate => _selectedSeason?.StartDate;
        private DateTime? MaxJoinDate => _selectedSeason?.EndDate;

        protected override async Task OnInitializedAsync()
        {
            await base.OnInitializedAsync();
            await Task.WhenAll(LoadTeamsAsync(), LoadSeasonsAsync());
        }

        private async Task OnSearchChanged(string value)
        {
            _searchTerm = value;
            await LoadTeamsAsync();
        }

        private async Task LoadTeamsAsync()
        {
            _loadingTeams = true;
            StateHasChanged();

            try
            {
                var response = await PlayerService.GetAvailableTeamsForPlayer(PlayerId, _searchTerm);

                if (response.IsSuccess && response.Data != null)
                {
                    _teams = response.Data;
                    if (_selectedTeam is not null && !_teams.Any(t => t.Id == _selectedTeam.Id))
                        _selectedTeam = null;
                }
                else
                {
                    _teams.Clear();
                    _selectedTeam = null;
                    Snackbar.Add(response.Message ?? "Failed to load available teams.", Severity.Error);
                }
            }
            finally
            {
                _loadingTeams = false;
                StateHasChanged();
            }
        }

        private async Task LoadSeasonsAsync()
        {
            _loadingSeasons = true;
            StateHasChanged();

            try
            {
                var response = await SeasonService.GetSeasons(new PaginationRequest { Page = 1, PageSize = 100 });

                if (response.IsSuccess && response.Data != null)
                    _seasons = response.Data.ToList();
                else
                    Snackbar.Add(response.Message ?? "Failed to load seasons.", Severity.Error);
            }
            finally
            {
                _loadingSeasons = false;
                StateHasChanged();
            }
        }

        private void SelectTeam(AvailableTeamDTO team)
        {
            _selectedTeam = _selectedTeam?.Id == team.Id ? null : team;
        }

        private string TeamRowClass(AvailableTeamDTO team, int _)
            => _selectedTeam?.Id == team.Id ? "picker-row-selected" : "picker-row";

        private void OnSeasonChanged(SeasonDTO? season)
        {
            _selectedSeason = season;
            if (_joinDate.HasValue && season != null &&
                (_joinDate.Value.Date < season.StartDate.Date || _joinDate.Value.Date > season.EndDate.Date))
            {
                _joinDate = null;
            }
        }

        private void Cancel() => MudDialog.Cancel();

        private bool CanSave => _selectedTeam is not null
            && _selectedSeason is not null
            && _joinDate.HasValue
            && !_saving;

        private async Task Save()
        {
            if (!CanSave) return;

            _saving = true;
            StateHasChanged();

            try
            {
                var cmd = new AssignPlayerToTeamCommand
                {
                    TeamId = _selectedTeam!.Id,
                    PlayerId = PlayerId,
                    SeasonId = _selectedSeason!.Id,
                    JoinDate = _joinDate!.Value,
                    JerseyNumber = _jerseyNumber
                };

                var result = await TeamService.AssignPlayerToTeam(_selectedTeam.Id, cmd);

                if (!result.IsSuccess)
                {
                    foreach (var error in result.Errors?.Any() == true
                        ? result.Errors
                        : new List<string> { result.Message ?? "Failed to assign player to team." })
                        Snackbar.Add(error, Severity.Error);
                    return;
                }

                MudDialog.Close(DialogResult.Ok(true));
            }
            finally
            {
                _saving = false;
                StateHasChanged();
            }
        }
    }
}
